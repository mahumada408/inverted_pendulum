...
class StateFeedback:
    def __init__(self, A, B, C, D, Q, R, S=None, **kwargs):
        ...
        Tinv = np.vstack((np.hstack((self.A - np.eye(self.nx), self.B)),np.hstack((self.C, np.zeros((self.C.shape[0], self.nu))))))

        T = np.linalg.solve(Tinv, np.eye(Tinv.shape[0])).dot(np.vstack((np.zeros((self.nx, 1)), np.ones((self.ny, 1)))))
